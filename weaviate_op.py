...
def metal_query(self,filters_str=None,return_fields=None,context={},limit=100,include_vector=False,return_raw=False,with_metadata=False,query_vector=None,target_vector=None,simplify_unique=True):
    try:
...
def resolve_load(col,to_load):
    ...
    for obj in to_load:
        ...
        is_prop_or_ref_array = __(keys_cleaned).apply(lambda x: x in prop_names+ref_names)
...
def resolve_ref_uuid_and_metal_load(col,ready_objs):
    ...
    to_load_ready=__(ready_objs).apply(lambda x: ({'prop':__(x['obj']).get(prop_names),
                                                   ...
                                                   'vect': __(x['obj']).get('vect')}))

    uuids=col.batch_load(to_load_ready)
    ...
    if len(to_load_opp_refs) > 0:
        col.batch_load(to_load_opp_refs)
    return uuids
...
def group_by_meta(data):
    ...
    for item in data:
        meta_key = tuple(item['$metal_meta$'].items())
...
def check_naming(keys,prop_names,ref_names):
    ...
    is_prop=__(keys).apply(lambda x: x in prop_names+ref_names)
    if is_pure_ref:
        return 'ref'
    if is_prop:
        return 'prop'
    ...
    if len(refs) > 0:
        ...
        for group in group_by_clt:
            ...
            if clt_name not in buffer_clt:
                from_clt=self.client_parent().get_metal_collection(clt_name)
                buffer_clt[clt_name]=from_clt
            else:
                from_clt=buffer_clt[clt_name]
            refs=[i['ref'] for i in refs]
